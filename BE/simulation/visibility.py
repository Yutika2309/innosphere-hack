...
def ensure_visibility(row: MutableMapping[str, Any], default: str = PRIVATE) -> Dict[str, Any]:
    """Return a dict copy of row with a valid visibility column."""
    out = dict(row or {})
    out[VISIBILITY_COLUMN] = normalise_visibility(out.get(VISIBILITY_COLUMN), default=default)
    return out


def ensure_rows_visibility(rows: Iterable[Mapping[str, Any]], default: str = PRIVATE) -> List[Dict[str, Any]]:
...
def is_public(row: Mapping[str, Any]) -> bool:
    ...
    return normalise_visibility((row or {}).get(VISIBILITY_COLUMN), default=PRIVATE) == PUBLIC
...
def force_world_news_rows_public(rows: Iterable[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    ...
    *,
    ...
    for row in rows or []:
        ...
        if not is_private(row_dict):
            continue
        if actor_type is not None and str(row_dict.get("actor_type", "")) != str(actor_type):
            continue
...
def normalise_dataframe_visibility(df: pd.DataFrame, default: str = PRIVATE) -> pd.DataFrame:
    ...
    out[VISIBILITY_COLUMN] = out[VISIBILITY_COLUMN].apply(lambda value: normalise_visibility(value, default=default))
    return out
