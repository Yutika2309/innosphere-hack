...
def _campaigns_for_bank(campaigns: List[Dict[str, Any]], bank_id: str) -> List[Dict[str, Any]]:
    return [c for c in campaigns if c.get("bank_id") == bank_id]


def _apply_db_scenario_state(bank: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    enable_db_improvements = bool(context.get("enable_db_improvements", scenario_mode == "improved"))
    ...
    resolved_bank["scenario_state"] = "improved_state" if use_improved_state else "current_state"
        ...
        actions.append(
            {
                ...
                "amount": amount,
                "currency": "EUR",
                "target_bank": persona.get("preferred_bank") or public.get("top_public_bank", "B001"),
            }
        )
        actions.append({"action": "compare_bank_offers", "count": public.get("visible_offer_count", 1)})
    else:
        actions.append({"action": "no_action", "reason": "low_current_need"})

    summary_update = f"Prev: {prev_summary[:120]} | NewActions: {_safe_json(actions)[:240]}"
    return {
        "actions": actions,
...
def bank_decision_tool(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if demand_index > 0.65:
        ...
        actions.append({"action": "adjust_interest_rate", "new_rate": round(rate + adj, 2)})
        actions.append({"action": "publish_offer", "segment": "prime_and_near_prime_germany"})
    elif demand_index < 0.35:
        actions.append({"action": "tighten_underwriting", "reason": "weak_market_signal"})
    else:
        actions.append({"action": "no_action", "reason": "stable_signal"})

    if _feedback_contains(feedback, "Faster approval") or _feedback_contains(feedback, "approval"):
        actions.append({"action": "improve_approval_speed", "reason": "customer_feedback"})
...
def generate_bank_offer_tool(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    scenario_mode = str(bank.get("scenario_mode") or context.get("scenario_mode") or "baseline")
    ...
    db_improved_mode = is_db_offer and bool(context.get("enable_db_improvements", scenario_mode == "improved"))
    ...
    campaign_discount_bps = max([_as_float(c.get("rate_discount_bps"), 0.0) for c in bank_campaigns] or [0.0])
    relationship_discount_bps = _as_float(bank.get("relationship_discount_bps"), 0.0)
    relationship_discount = relationship_discount_bps if consumer.get("preferred_bank") == bank_id else 0.0
    ...
    interest_rate = round(max(3.0, base_rate + risk_adjustment - (campaign_discount_bps + relationship_discount) / 100.0), 2)
    ...
    approved_amount = requested_amount if approved else min(requested_amount, affordability_cap)
    ...
    offer = {
        ...
        "approval_time_days": min(
            [_as_int(c.get("promised_approval_time_days"), _as_int(bank.get("average_approval_time_days"), 3)) for c in bank_campaigns]
            or [_as_int(bank.get("average_approval_time_days"), 3)]
        ),
        "documentation_friction_score": _as_float(bank.get("documentation_friction_score"), 0.3),
        ...
        "abandoned_application_followup_enabled": bool(bank.get("abandoned_application_followup_enabled", False)),
        "instant_document_validation_enabled": bool(bank.get("instant_document_validation_enabled", False)),
        ...
        "comparison_portal_optimised": bool(bank.get("comparison_portal_optimised", False)),
    ...
    db_improved_bonus = 0.04 if str(bank_id) == deutsche_bank_id and bool(context.get("enable_db_improvements", scenario_mode == "improved")) else 0.0
...
def consumer_funnel_decision_tool(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    best_offer = max(approved_offers, key=lambda o: _as_float(o.get("offer_score"), 0.0), default=None)
    preferred_offer = next((o for o in approved_offers if o.get("bank_id") == preferred_bank), None)
    ...
    if current_stage in TERMINAL_STAGES:
        actions.append({"action": "no_action", "reason": f"terminal_stage_{current_stage}"})
    elif current_stage == "lead":
        campaign_lift = sum(_as_float(c.get("expected_lead_lift"), 0.0) for c in campaigns if c.get("bank_id") == preferred_bank)
        if loan_need + campaign_lift >= 0.55:
            next_stage = "aware"
            actions.append({"action": "notice_loan_offer", "source": "marketing_or_existing_need"})
        else:
            next_stage = "dropped"
            drop = True
        ...
        if not offers:
            ...
            actions.append({"action": "request_prequalification", "credit_bureau": "SCHUFA"})
        elif approved_offers:
            next_stage = "prequalified"
            actions.append({"action": "compare_prequalified_offers", "offer_count": len(approved_offers)})
        else:
            next_stage = "rejected"
            drop = True
        ...
        if best_offer is None:
            ...
            actions.append({"action": "start_application", "target_bank": selected_bank})
    elif current_stage == "application_started":
        friction = _as_float((best_offer or {}).get("documentation_friction_score"), 0.3)
        if friction > 0.45 and documentation_tolerance < 0.5:
            ...
            actions.append({"action": "submit_application", "target_bank": selected_bank or (best_offer or {}).get("bank_id")})
        ...
        if approval_days > 3 and urgency > 0.65:
            ...
            next_stage = "approved"
            actions.append({"action": "receive_credit_approval", "bank_id": (best_offer or {}).get("bank_id")})
    elif current_stage == "approved":
        next_stage = "offer_received"
        actions.append({"action": "receive_final_offer", "bank_id": (best_offer or {}).get("bank_id")})
        ...
        if best_offer is None:
            ...
            selected = preferred_offer if preferred_offer and brand_loyalty >= 0.75 else best_offer
            ...
            if preferred_offer and best_offer.get("bank_id") != preferred_bank and price_sensitivity > 0.65 and selected_rate - best_rate > 0.25:
                next_stage = "dropped"
                drop = True
                drop_reason = "competitor_offer_better"
                selected_bank = best_offer.get("bank_id")
                actions.append({"action": "drop_from_funnel", "reason": drop_reason, "winner_bank": selected_bank})
            elif _as_float(selected.get("estimated_emi"), 0.0) > (_as_float(consumer.get("monthly_income"), 0.0) - _as_float(consumer.get("monthly_expense"), 0.0)) * 0.5:
                next_stage = "dropped"
                drop = True
        ...
        actions.append({"action": "sign_agreement_and_disburse", "bank_id": selected_bank or (best_offer or {}).get("bank_id")})
    ...
    if not drop and next_stage not in TERMINAL_STAGES:
        ...
        if best_offer:
            friction_signal += _as_float(best_offer.get("digital_friction_score"), 0.0) * (1.0 - digital_comfort) * 0.08
            friction_signal += _as_float(best_offer.get("documentation_friction_score"), 0.0) * (1.0 - documentation_tolerance) * 0.08
            friction_signal += max(0.0, _as_float(best_offer.get("interest_rate"), 0.0) - 8.0) * price_sensitivity * 0.01
        ...
        if friction_signal > 0.18 and threshold < friction_signal - 0.18:
            next_stage = "dropped"
            drop = True
            if best_offer and _as_float(best_offer.get("interest_rate"), 0.0) > 8.0 and price_sensitivity > 0.65:
                drop_reason = "rate_too_high"
            elif best_offer and _as_float(best_offer.get("digital_friction_score"), 0.0) > 0.4:
    ...
    selected_offer = None
    if selected_bank:
        selected_offer = next((o for o in offers if o.get("bank_id") == selected_bank), None)
...
def timestep_summary_tool(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if metrics:
        submitted = metrics.get("cumulative_applications_submitted", metrics.get("applications_submitted", 0))
