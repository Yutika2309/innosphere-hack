...
def _take_recent(rows: Iterable[Dict[str, Any]] | None, limit: int = 50) -> List[Dict[str, Any]]:
...
def latest_public_summary(sim: Any) -> Dict[str, Any]:
    ...
    if not latest:
        return {
            ...
            "window_size": getattr(getattr(sim, "cfg", object()), "public_summary_window_timesteps", 10),
        }
    return {
        "public_information_summary": latest.get("public_information_summary", "No public summary available."),
...
def committed_public_rows(sim: Any, table_name: str, limit: int = 100) -> List[Dict[str, Any]]:
    rows = getattr(sim, f"{table_name}_rows", []) or []
    return _take_recent([r for r in rows if isinstance(r, dict) and _visibility_public(r)], limit)
...
def build_bank_contexts(sim: Any) -> List[Dict[str, Any]]:
    ...
    for bank in banks:
        ...
        contexts.append({
            ...
            "bank_private_summary": getattr(sim, "private_summaries", {}).get(bank_id, ""),
            ...
            "own_previous_bank_actions": _take_recent([r for r in getattr(sim, "bank_actions_rows", []) if r.get("bank_id") == bank_id], 25),
            "own_previous_bank_offers": _take_recent([r for r in getattr(sim, "bank_offers_rows", []) if r.get("bank_id") == bank_id], 25),
            "own_previous_marketing_actions": _take_recent([r for r in getattr(sim, "marketing_actions_rows", []) if r.get("bank_id") == bank_id], 25),
...
def build_customer_contexts(sim: Any) -> List[Dict[str, Any]]:
    ...
    public_summary = latest_public_summary(sim)
    dashboard = latest_marketplace_dashboard(sim)
    public_offers = [canonical_public_offer_view(r) for r in committed_public_rows(sim, "bank_offers", 80)]
    contexts: List[Dict[str, Any]] = []
    for consumer in getattr(sim, "consumers", []) or []:
        ...
        contexts.append({
            ...
            "own_private_summary": getattr(sim, "private_summaries", {}).get(cid, ""),
            "public_information_summary": public_summary["public_information_summary"],
            "public_summary_from_timestep": public_summary.get("summary_from_timestep"),
            "public_summary_to_timestep": public_summary.get("summary_to_timestep"),
            "marketplace_dashboard": dashboard,
            ...
            "source_preferences": {
                "marketplace_reliance": getattr(cfg, "default_marketplace_reliance", 0.5),
                "public_news_reliance": getattr(cfg, "default_public_news_reliance", 0.3),
                "relationship_bank_reliance": getattr(cfg, "default_relationship_bank_reliance", 0.4),
                "branch_advice_preference": getattr(cfg, "default_branch_advice_preference", 0.2),
                "digital_comparison_preference": getattr(cfg, "default_digital_comparison_preference", 0.5),
            },
...
def build_marketplace_context(sim: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "public_summary_from_timestep": public_summary.get("summary_from_timestep"),
        "public_summary_to_timestep": public_summary.get("summary_to_timestep"),
        "committed_public_bank_offers": [canonical_public_offer_view(r) for r in committed_public_rows(sim, "bank_offers", 120)],
        "committed_public_bank_actions": committed_public_rows(sim, "bank_actions", 80),
        "committed_public_marketing_actions": committed_public_rows(sim, "marketing_actions", 80),
