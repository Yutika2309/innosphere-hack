...
## Persistent baseline/improved run directories

The simulation now writes scenario runs into persistent directories instead of mixing every run into one output folder. The default layout is:
...
Baseline and improved simulations no longer overwrite each other. Each scenario has a `latest_run.json` pointer so tools can find the newest run after ADK restarts.

## Restart-safe summaries

Run summaries are generated from persisted CSV files using `run_summary.py`, not from in-memory engine state. This means the agent can summarise the latest baseline run, summarise the latest improved run, or compare both after the ADK process restarts.

Relevant helpers:

- `summarise_run_dir(run_dir)`
- `summarise_latest_run(runs_root_dir, scenario_name)`
- `compare_run_dirs(baseline_run_dir, improved_run_dir)`
- `compare_latest_runs(runs_root_dir, "baseline", "improved")`
