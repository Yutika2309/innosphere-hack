...
def build_actor_summary(
    actor_id: str, rows: List[Dict[str, Any]], previous_summary: str = ""
) -> Dict[str, Any]:
    rows = list(rows or [])
    source_tables = sorted({str(r.get("source_table", "unknown")) for r in rows})
    timesteps = [
        int(r.get("timestep")) for r in rows if str(r.get("timestep", "")).isdigit()
    ]
    ...
    actor_type = (
        "customer"
        if _is_customer_id(actor_id)
        else "bank" if _is_bank_id(actor_id) else "unknown"
    )

    snippets = []
    for row in rows[-12:]:
        snippets.append(
            f"T{row.get('timestep')}: {row.get('source_table')} - {_row_text(row)[:220]}"
        )
...
def update_private_summaries(
    ...
    for actor_id, rows in grouped.items():
        summary_row = build_actor_summary(
            actor_id, rows, current_summaries.get(actor_id, "")
        )
...
def build_private_summary_rows(
    ...
    for actor_id, actor_rows in grouped.items():
        rows.append(
            build_actor_summary(
                actor_id, actor_rows, current_summaries.get(actor_id, "")
            )
        )
    return rows
...
# Customer private summaries include consumer_actions.decision_reason. Bank private summaries include reputation and offer-performance signals where present.
