...
try:
    from .run_paths import resolve_run_paths, write_latest_pointer
    from .run_summary import summarise_run_dir, compare_run_dirs
except Exception:  # pragma: no cover
    from run_paths import resolve_run_paths, write_latest_pointer  # type: ignore
    from run_summary import summarise_run_dir, compare_run_dirs  # type: ignore
...
class LendingWorldSimulation:
    def __init__(self, cfg: Any):
        ...
        if str(getattr(cfg, "scenario_output_mode", "separate_directories")) == "separate_directories":
            self.run_paths = resolve_run_paths(cfg)
            self.output_dir = self.run_paths.run_dir
            self.output_dir.mkdir(parents=True, exist_ok=True)
            if bool(getattr(cfg, "write_latest_run_pointer", True)):
                write_latest_pointer(self.run_paths)
        ...
        if bool(getattr(self.cfg, "write_run_metadata", True)):
            self._write_run_metadata()


    # ------------------------------------------------------------------
    # Persistent run metadata and summaries
    # ------------------------------------------------------------------

    def _write_run_metadata(self) -> None:
        if not getattr(self, "run_paths", None):
            return
        metadata = {
            "run_id": self.run_paths.run_id,
            "scenario_name": self.run_paths.scenario_name,
            "run_dir": str(self.output_dir),
            "timesteps": getattr(self.cfg, "timesteps", None),
            "seed": getattr(self.cfg, "seed", None),
            "scenario_mode": getattr(self.cfg, "scenario_mode", None),
            "enable_db_improvements": getattr(self.cfg, "enable_db_improvements", None),
            "deutsche_bank_id": getattr(self.cfg, "deutsche_bank_id", "B001"),
            "max_active_public_offers": getattr(self.cfg, "max_active_public_offers", None),
            "max_active_public_offers_per_bank": getattr(self.cfg, "max_active_public_offers_per_bank", None),
            "max_new_public_offers_per_bank_per_timestep": getattr(self.cfg, "max_new_public_offers_per_bank_per_timestep", None),
        }
        self.run_paths.metadata_path.parent.mkdir(parents=True, exist_ok=True)
        self.run_paths.metadata_path.write_text(json.dumps(metadata, indent=2, ensure_ascii=False), encoding="utf-8")

    def summarise_persisted_run(self, run_dir: str | Path | None = None) -> Dict[str, Any]:
        return summarise_run_dir(Path(run_dir) if run_dir else self.output_dir)

    def compare_persisted_runs(self, baseline_run_dir: str | Path, improved_run_dir: str | Path) -> Dict[str, Any]:
        return compare_run_dirs(baseline_run_dir, improved_run_dir)
