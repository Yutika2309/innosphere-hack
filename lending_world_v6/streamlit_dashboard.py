...
try:
    from run_summary import load_latest_run_dir, compare_latest_runs
except Exception:  # pragma: no cover
    load_latest_run_dir = None
    compare_latest_runs = None
...
EXPECTED_FILES = {
    ...
    "customer_lifecycle": "customer_lifecycle_snapshot.csv",
    ...
    "marketplace_dashboard": "marketplace_dashboard.csv",
    "invalid_rows": "invalid_rows.csv",
    "consumer_actions": "consumer_actions.csv",
}
...
with st.sidebar:
    ...
    st.subheader("Scenario run browser")
    runs_root_text = st.text_input("Runs root", value="runs")
    scenario_name = st.selectbox("Scenario", ["baseline", "improved"], index=0)
    latest_dir = load_latest_run_dir(runs_root_text, scenario_name) if load_latest_run_dir else None
    default_output = str(latest_dir or OUTPUT_DIR)
    output_dir_text = st.text_input("Output/run directory", value=default_output)
    output_dir = Path(output_dir_text)
    if compare_latest_runs and st.checkbox("Compare latest baseline vs improved", value=False):
        st.json(compare_latest_runs(runs_root_text, "baseline", "improved"))
